<template>
  <div class="container">
    <div class="hello">
      <Nav class="nav"/>
      <Table table="categories" :list="categories" @addItem="showModal" @deleteItem="deleteItem($event)" @editItem="editItem($event)" />
      <transition name="modal">
        <div v-if="isOpen" class="modal-shadow" @click.self="closeModal">
            <form class="modal" @submit.prevent="onSubmit" >
                <div class="modal-close" @click="closeModal"> ╳</div>
               
                    <h3 class="modal-title">{{edit ? 'Изменить категорию' : 'Добавить категорию'}}</h3>
...
export default {
  ...
  methods: {
    deleteItem(id){
      axios.delete(`http://localhost:4201/api/Category/${id}`).then(()=>{
           window.location.reload(false); 
      })
    },
     editItem(id){
      let el = this.categories.find(item => item.id == id);
    ...
    onSubmit() {
      if (this.edit) {
       axios.post(`http://localhost:4201/api/Category/`, {"id":this.edit, "name": this.category}).then((response)=>{
           window.location.reload(false);
           this.edit=false; 
           this.closeModal()
      })
      } else {
        axios.post('http://localhost:4201/api/Category', {"name": this.category}).then((response)=>{
           window.location.reload(false); 
           this.closeModal()
      })
      }
